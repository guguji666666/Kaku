...
Run `kaku ai`, keep Auth Type set to API key, and enter the OpenAI-compatible Base URL, API key, Simple Model, and Deep Model manually. The URL must be OpenAI-compatible (`/v1/chat/completions`).
...
This removes Kaku-managed shell and tmux integration, Kaku-managed git delta defaults, selected Kaku state, and managed theme blocks in `~/.config/kaku/kaku.lua`. User-authored Lua outside managed blocks is preserved. Run `kaku init` again if you want shell integration back.
...
See [CLI Reference](https://kaku.fun/docs/cli) for full reference.
...
`Cmd + Opt + Ctrl + K` uses the physical QWERTY K position. On Colemak, this corresponds to a different key. Remap it:
