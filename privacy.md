...
- **AI assistant.** When you enable Kaku Assistant with `kaku ai`, prompts go directly to the OpenAI-compatible endpoint you configured, using the API key or CLI login you supplied. Kaku does not proxy, relay, log, or retain these requests, and there is no default provider: the assistant is inert until you configure one. What that provider does with your prompts is governed by its own privacy policy, not by Kaku. The requests your shell can trigger, such as the `#` prompt and the automatic command fix, are authenticated with a local capability, so text that merely prints in a pane cannot spend your credentials on requests you did not ask for.
...
Last reviewed for Kaku v0.16.0.
